...
public class PSP1_3a{
    ...
    public static double valoresXX(int b[][], int d[][]){
        int x, a, sumxx=0;
        for(x=0; x<10; x++){
            for(a=0; a<1; a++){
                d[a][x] = b[a][x] * b[a][x];
    ...
    public static double valoresXY(int b[][], int c[][], int e[][]){
        int x, y, a=0, sumxy=0;
        for(x=0; x<1; x++){
            for(y=0; y<10; y++){
                e[x][y] = b[x][y] * c[x][y];
    ...
    public static double valoresYY(int c[][], int f[][]){
        int y, a, sumyy=0;
        for(y=0; y<10; y++){
            for(a=0; a<1; a++){
                f[a][y] = c[a][y] * c[a][y];
    ...
    public static double b1(double sumxy, double yavg, double sumxx, double xavg){
        double b01 = 0;
        b01 = (((sumxy) - (10 * yavg)) / ((sumxx) - (10 * (xavg*xavg))));
    ...
    public static double rxy(double sumxy, double sumx, double sumy, double sumxx, double sumyy){
        double vrxy = 0, r2 = 0;
        //r2 = 0;
        vrxy = ((10 * sumxy) - ((sumx * sumy))) / Math.sqrt(((10*sumxx) - ((sumx*sumx)) * (10*sumyy)) - (sumx*sumx));
        //r2 = rxy * rxy;
        System.out.println("\n rxy \n" +vrxy);
        //System.out.println(" \n r^2 \n" +r2);
        return vrxy;
